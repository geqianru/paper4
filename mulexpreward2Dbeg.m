function [minLCCnew, R, P] = mulexpreward2Dbeg(alpha, betat, act, minLCC, redesigncost, D0, r, cp, cr, t, T)
...
reverse = ones(1, dim);
   ...
   if(t == T)
       ...
       for d1 =  0 : failurebound(t)
           ...
           probt(d1 + 1, 1) = -1;
               for d2 = 0 : failurebound(t)
                   ...
                   if and(probt(d1 + 1, d2 + 2) * probt1 > 1e-16, stat(d1 + 1, d2 + 1)<= numstate)
                       probstat(xt) = probt(d1 + 1, d2 + 2) * probt1;
                       statatem(xt) = stat(d1 + 1, d2 + 1);
                       xt = xt + 1;
                   elseif and(probt(d1 + 1, d2 + 2) * probt1 <= 1e-16, xt == 1)
                       break;
                   elseif or (and(probt(d1 + 1, d2 + 2) * probt1 <= 1e-16, true(probt(d1 + 1, d2 + 2)< probt(d1 + 1, d2 + 1) ) ), and(stat(d1 + 1, d2 + 1) > numstate, xt > 1) ) 
    ...
    R = sum(alpha) * cr / betat ;
    P = pencostdim(sum(alpha), betat, r, D0, cp, failurebound(t + 1), dim);
    minLCCnew = R + P + g1 + g2;       
end